...
public:
	explicit Player(Atlas* atlas_idle_up, Atlas* atlas_idle_down, Atlas* atlas_idle_left, Atlas* atlas_idle_right,
		...
		anim_idle_down.add_frame(atlas_idle_up);
	...
	void on_update(float delta) {
		...
		if (velocity.approx(Vector2(0, 0))) {
			...
			if (abs(velocity.x) >= 0.0001f)
				facing = (velocity.x > 0) ? Player::Facing::Right : Player::Facing::Lest;
