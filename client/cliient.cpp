...
int id_player = 0;
std::atomic_int process_1 = -1;
std::atomic_int process_2 = -1;
...
void login_to_server(HWND hwnd) {
	client = std::make_shared<httplib::Client>(str_address);
	...
	(id_player == 1) ? (process_1 = 0) : (process_2 = 0);
	...
	std::thread([&]() {
		while (true) {
			...
			std::string body = std::to_string((id_player == 1) ? process_1 : process_2);
			httplib::Result result = client->Post(route, body, "text/plain");

			if (result && result->status == 200) {
				int progress = std::stoi(result->body);
				(id_player == 1) ? (process_2 = progress) : (process_1 = progress);
			}
...
int main(int argc, char** argv) {
	...
	SetWindowText(hwnd, _T("made in heaven"));
	...
	while(true) {
		while (peekmessage(&msg)) {
			...
			if (msg.message == WM_CHAR && idx_line < str_line_list.size()) {
				...
				if (str_line[idx_char] == msg.ch) {
					...
					(id_player == 1) ? process_1++ : process_2++;
		...
		if (stage == Stage::Waiting) {
			...
			if ((id_player == 1 && process_1 >= num_total_char)
				|| (id_player == 2 && process_2 >= num_total_char)) {
				stop_audio(_T("bgm"));
			...
			else if ((id_player == 1 && process_2 >= num_total_char)
				|| (id_player == 2 && process_1 >= num_total_char)) {
				stop_audio(_T("bgm"));
				MessageBox(hwnd, _T("输光光!"), _T("游戏结束"), MB_OK | MB_ICONINFORMATION);
				exit(0);
			}

			player_1.set_target(path.get_position_at_progess((float)process_1 / num_total_char));
			player_2.set_target(path.get_position_at_progess((float)process_2 / num_total_char));
